...
class ParserTest {
    ...
    fun `new single keys and text objects with delimiters`() {
        ...
        assertEquals(Command(Action.LINE_PERCENT, 50, false, "<"), complete("<"))
        assertEquals(Command(Action.LINE_PERCENT, 25, true, "25<"), complete("25<"))
        assertEquals(Command(Action.GOTO_PERCENT, 50, false, ">"), complete(">"))
        assertEquals(Command(Action.GOTO_PERCENT, 30, true, "3>"), complete("3>"))
        invalid("0<")
